...
knitr::include_graphics(file.path(pathToImages, "Ariane_5ES_liftoff_with_ATV_4.png"))
```
:::
:::::::::
...
knitr::include_graphics(file.path(pathToImages, "VerteilungGleitpunktzahlen.png"))
```
...
Die arithmetischen Grundoperationen $+, -,  \cdot, /$ werden auf Rechenanlagen durch entsprechende "*Maschinenoperationen*" $+, -, \cdot, /$ ersetzt, die Maschinenzahlen in Maschinenzahlen überführen.
...
$$\tilde{x} \bot \tilde{y} = rd(x \bot y) = (x \bot y)\cdot(1+\epsilon), \qquad  |\epsilon| < eps$$

**Auf dieser Annahme fußt der Großteil moderner Rundungsfehleranalyse!**

*Insbesondere gilt:*

$$\tilde{x} + \tilde{y} = \tilde{x}, \qquad  \text{ für } |y| \leq \frac{|x|}{B} \cdot eps$$
...
$$
    \begin{aligned}
        rd(x_1 - x_2)   &= 0.123454 \cdot 10^{12} - 0.123446 \cdot 10^{12} \\ 
...
$$
    \begin{aligned}
        rd(x_1 - x_3)   &= 0.123454 \cdot 10^{12} - 0.123435 \cdot 10^{12} \\
