...
$A(x): 4 | x$; $B(X): x \text{ ist eine gerade Zahl}$; $C(x): 2 | x$
...
Sei $A(n)$ eine Aussage. Gilt nun$A(n_0)$ für ein $n_0 \in \mathbf{N}_0$ und darüberhinaus 
$\forall n \in \mathbf{N}_0: (n \geq n_0 \wedge A(n) \Rightarrow A(n+1))$ dann folgt daraus $\forall n \in \mathbf{N}_0: (n \geq n_0 \wedge A(n))$
...
C) Wenn a eine natürliche Zahl ist, dann ist $a^2$ durch 4 teilbar oder beim Teilen
von $a^2$ durch 4 bleibt der Rest 1.

D) Für alle reellen Zahlen z gilt: $z^2 > 0$ oder $z^2 = 0$.

E) Für alle positiven rationalen Zahlen $q$ gibt es eine rationale Zahl $w$ mit $w^2 = q$.
