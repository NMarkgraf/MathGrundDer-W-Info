...
c) Keine Äquibalentrelation, da z.B. $(1; 2) \in\; \leq_A$ aber $(2; 1) \,\not\in\; \leq_A$ (Nicht symmetrisch!)

d) Keine Äquibalentrelation, da z.B. $(2; 1) \in\; \geq_A$ aber $(1; 2) \,\not\in\; \geq_A$ (Nicht symmetrisch!)

e) Äquivalenzrelation! Da:
...
Mit der Funktion $x \mod n$ erhalten den Rest bei der Division mit Rest von $x$ geteilt durch $n$. Mit $\lfloor x \rfloor := \max \{k\in\mathbf{Z} \mid k\leq x\}$ gilt also:
$$x = \lfloor x / n \rfloor \cdot n + (x \mod n)$$
...
D. Es seien $\max(x,y) := \begin{cases}x &: x \geq y \\ y &: x < y\end{cases}$ und $\min(x,y) := -\max(-x,-y)$ sowie zwei beliebige Teilmengen $A$ und $B$ von $\mathbf{R}$ gegeben. 
