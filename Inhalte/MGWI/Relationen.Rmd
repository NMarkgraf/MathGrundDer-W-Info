...
| Elemnt-Notation  | Infix Notation  |
|:-:|:-:|
| $(1, 2) \in <$     | $1 < 2$    |
| $(1,2) \in \leq$   | $1 \leq 2$ |
| $(2,1) \in\;>$     | $2 > 1$    |
...
- $[0]_4 = A_0 = \{ x \in \mathbf{Z} \,|\, x = 0 \pmod 4\}$
- $[1]_4 = A_1 = \{ x \in \mathbf{Z} \,|\, x = 1 \pmod 4\}$
- $[2]_4 = A_2 = \{ x \in \mathbf{Z} \,|\, x = 2 \pmod 4\}$
- $[3]_4 = A_3 = \{ x \in \mathbf{Z} \,|\, x = 3 \pmod 4\}$
...
Es gilt dann $[x]_m = [y]_m$, falls $x \in [a]_m$ und auch $y \in [a]_m$. 
Das ist eine Umformulierung von $x = y = a \pmod m$ mit $a, x, y \in \mathbf{Z}$ und
$0 \leq a < m$.

Wir können das auf die Restklassen $[.]_m$ übertragen und schreiben:
...
Das Rechnen im Restklassenring $\mathbf{Z_n}$ enspricht als dem *normalen* Rechnen mit ganzen Zahlen, bei dem man nach jeder Operation ($+$, $-$, $\cdot$, $\div$) nur noch den Rest bei der Division (mit Rest) mit $n$ betrachtet.
...
::: {.small}
Eine **Hashfunktion** $H$ ist eine Abbildung aus einem Alphabet $\Sigma$ in einen 
Zahlenmenge (z.B. $\mathbf{Z_n}$). Zum Beispiel:

Neben wir an, wir haben ein Feld (Array) mit $n=8$ Einträgen. Die Schlüssen 
für dieses Feld sind Zeichenketten (zur Vereinfachung aus Großbuchstaben).

Die Funktion $\text{chr}(X)$ wandelt jeden Buchstaben des Alphabets ("A", "B", ... bis "Z") in die Zahlen $0$, $1$, ... bis $25$ um. 

Sei $w$ ein Wort mit Buchstanden aus dem Alphabet, so bezeichnen wir mit $w_i$ den
$i$-ten Buchstaben des Wortes. Für das Wort "XYZ" wäre das also 
$w_1 =$"X", $w_2 =$"Y" und $w_3=$"Z"

Eine mögliche Hashfunktion wäre dann z.B. $H(w) = \sum_{i} \text{chr}(w_i) \pmod n$.

$$H(WIEN)= \text{chr}(W) + \text{chr}(I) + \text{chr}(E) + \text{chr}(N) \pmod 7 = 22 + 8 + 4 + 13 \pmod 7 = 5 \pmod 7$$

Man könnte also die Informationen über "WIEN" ins 5. Feld ablegen. 
...
b) Die 10-stellige ISBN kann man als $a-bcd-efghi-p$ mit den Ziffern a,b,c,d,e,f,g,h,i und der Prüfziffer $p$ darstellen. Dabei erfüllt $p$ die Gleichung: 
$$(10\cdot a + 9\cdot b + 8\cdot c + 7\cdot d + 6\cdot e + 5\cdot f + 4\cdot g + 3\cdot h + 2\cdot i + p) \mod 11 = 0$$
 
    i) Ist 3-658-02803-3 eine gültige 10-stellige ISBN? (Und welches Buch ist es?)
 
    ii) Ein Einzelfehler geschieht an der 2. Stelle und es wird 3-558-02803-3. Wird dieser Fehler erkannt?
...
b) Die Antworten lauten:
    i) Ja. (Das Buch heißt übrigens *Angewandte Wirtschaftsstatistik: Daten und Zufall (FOM-Edition)* von den Autoren Karsten Lübke und Martin Vogt. Hier berät sie, ausserhalb der Klausuren, ggf. die Buchhandlung ihres Vertrauens!)
    ii) Ja, denn den Prüfgleichung liefert nicht $0$, sondern $2$ modulo $11$.
...
gf_line(y ~x, size=1.5, color = "blue", data = M1) %>%
    gf_line(y ~ x, size=1.5, color = "blue", data=M2) %>%
    gf_line(y ~ x, size=1.5, color = "blue", data=M3) %>%
...
gf_line(y ~x, size=1.5, color = "white", data = M1) %>%
    gf_line(y ~ x, size=1.5, color = "red", data=M2) %>%
