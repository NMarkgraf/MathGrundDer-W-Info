...
Eine **zweistellige** oder auch **binäre Relation** zwischen zwei Mengen $A$ und $B$ ist eine Teilmenge des kartesischen Produkts:
$$R \subseteq A \times B$$
...
- $\displaystyle \mathbf{D}_R =\left\{a \in A \,\left|\, \exists b \in B : (a,b) \in R \right.\right\}$ den [Definitionsbereich]{.cstrong}

- $\displaystyle \mathbf{W}_R =\left\{b \in B \,\left|\, \exists a \in A : (a,b) \in R \right.\right\}$ den [Wertebereich]{.cstrong}

#### {.examples}

- Die Leeremenge $\emptyset$ als Teilmenge von $A \times B$ aufgefasst als Relation heißt [Nullrelation]{.cstrong}.
...
Eine zweistellige Relation $R\subseteq A \times A$ *auf* einer Menge $A$ heißt [homogene Relation]{.cstrong}.

#### Infix Notation von homogenen Relationen {.definition}

Bei einer *homogene* Relation $R \subseteq A \times A$ schreibt mensch anstelle von $(x, y) \in R$ auch $xRy$.
...
Eine (homogene) Relation $\sim$ auf einer Menge A heißt [Äquivalenzrelation]{.cstrong}, falls für alle $a,b,c \in A$ gilt:
...
aller zu $a$ *aquivalenten* Elemente nennen wir die [Äquivalenzklasse]{.cstrong} von $a$ (bzgl. $\sim_n$ und $\mathbf{Z}$). Jedes Element der Klasse nennen wir einen [Repräsentanten]{.cstrong} dieser Klasse. 



Die Menge aller Äquivalenzklassen bezeichnen wir mit 
$$\mathbf{Z_n} := \mathbf{Z}/n \mathbf{Z} := \mathbf{Z}/\sim_n := \{ [a]_n\,|\,a \in \mathbf{Z}\}.$$ 
Zusammen mit der Addition $[a]_n + [b]_n = [a+b]_n$ und Multiplikation $[a]_n \cdot [b]_n = [a \cdot b]_n$ nennen wir den [Restklassenring $Z$ modulo $n$]{.cstrong} $(\mathbf{Z_n}, +, \cdot)$.
