...
Eine (reellwertige) Funktion ist **(folgen-)stetig**, wenn für alle $h>0$ gilt:
...
Eine *stetige Funktion* $f$, die zwischen zwei Stellen $x_0$ und $x_1$ das 
Vorzeichen wechselt, also $f(x_0)\cdot f(x_1) < 0$, hat zwischen 
$x_0$ und $x_1$ mindestens eine Nullstelle.
...
gf_fun( cos(0.4*(x+2))  ~ x, color = "blue", xlim=c(-2, 6)) %>%
  gf_hline(color = my_zero_line_color, yintercept = ~ 0, show.legend = FALSE) %>%
  gf_point(1 ~ -2, color = my_line_color) %>%
  gf_point(-1 ~ 6, color = my_line_color) %>%
  gf_path(c(0, 1) ~ c(-2, -2), linetype = "dotted") %>%
  gf_path(c(0, -1) ~ c(6, 6), linetype = "dotted") %>%
  gf_point(0 ~ 2.5*(3.14/2)-2, color = my_point_color) %>%
  gf_theme(theme_bw(base_size = my_size*11)) 
```
...
gf_function( fun = my_fkt, color = "blue", xlim=c(-1, 6)) %>%
  gf_hline(color = my_zero_line_color, yintercept = ~ 0, show.legend = FALSE) %>%
  gf_point(my_fkt(0) ~ 0, color = my_line_color) %>%
  gf_text(my_fkt(0) ~ 0, label = "P", size = 4, angle = 0, hjust = 0, nudge_x = 0.1, nudge_y = 0.1) %>%
  gf_point(my_fkt(5) ~ 5, color = my_line_color) %>%
  gf_text(my_fkt(5) ~ 5, label = "Q", size = 4, angle = 0, hjust = 0, nudge_x = 0.1, nudge_y = -0.1) %>%
  gf_abline(slope = ((my_fkt(5) - my_fkt(0)) / (5 - 0)), intercept = my_fkt(0), color = my_sec_color) %>%
  gf_theme(theme_bw(base_size = my_size*11)) 
```
...
Die Sekante wird dann durch den folgenden Funktionsterm beschrieben:
$$m_s \cdot x + b_s$$
...
$f''(-2) = -18 < 0$, also ist bei $x_1 = -2; y_1 = 30$ ein lokales Maximum.

$f''(4) = 18 > 0$, also ist bei $x_2=4;\, y_2 = -78$ ein lokales Minimum.

Wegen $f''(x) = 0 \Leftrightarrow x=1$ und $f'''(1)=6 \neq 0$ gibt es bei
$x=1$ einen Wendepunkt bei $(1| -24). 
...
gf_function( fun = my_G, color = "green", xlim = c(0, 20.5)) %>%
  ...
  gf_label(10 ~ 1.5, label = "E(x)", size = 4, angle = 0, hjust = 0, nudge_x = -0.5, nudge_y = 0.5) %>% 
  gf_label(my_K(1.5) ~ 1.5, label = "K(x)", size = 4, angle = 0, hjust = 0, nudge_x = -0.5, nudge_y = 0.5) %>% 
  gf_label(my_G(1.5) ~ 1.5, label = "G(x)", size = 4, angle = 0, hjust = 0, nudge_x = -0.5, nudge_y = 0.5) %>% 
  gf_label((my_G(12.5) + 15) ~ 12.5, label = "max. Gewinn", size = 4, angle = 0, hjust = 0, nudge_x = -0.5, nudge_y = 0.5) %>%
  gf_label(-15 ~ 5.6, label = "Break Even Point", size = 4, angle = 0, hjust = 0, nudge_x = -0.5, nudge_y = 0.5) %>%
...
gf_function( fun = my_DK, color = "red", xlim = c(5, 20.)) %>%
  gf_function( fun = my_G, color = "blue", xlim = c(5, 20)) %>%
#  gf_function( fun = my_DE, color = "green", xlim = c(5, 20)) %>%
#  gf_function( fun = my_DG, color = "orange", xlim = c(5, 20)) %>%
  gf_point(my_DK(10) ~ 10, color = "red", size = 2) %>%
  gf_point(my_G(12.5) ~ 12.5, color = "blue", size = 2) %>%

  gf_refine(scale_x_continuous(breaks = seq(5,20,2.5))) %>%
  gf_refine(scale_y_continuous(breaks = c(0, my_DK(10), 10, 36))) %>%
  gf_hline(yintercept = 0, color = "black") %>%
  gf_theme(theme_bw(base_size = my_size*11)) 
...
#  gf_function( fun = my_E, color = "blue", xlim = c(0, 7)) %>%
...
$DK'(x) = 0{,}5 - \frac{18}{x^2} = 0 \Leftrightarrow x_{BO}= x_1=6, x_2=-6$
...
gf_function( fun = my_DK, color = "red", xlim = c(2, 6.5)) %>%
  gf_function( fun = my_G, color = "blue", xlim = c(2, 6.5)) %>%
#  gf_function( fun = my_DE, color = "green", xlim = c(5, 20)) %>%
#  gf_function( fun = my_DG, color = "orange", xlim = c(5, 20)) %>%
  gf_point(my_DK(6) ~ 6, color = "red", size = 2) %>%
  gf_point(my_G(4) ~ 4, color = "blue", size = 2) %>%
